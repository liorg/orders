    ...
    public class FController : Controller
    {
        ...
        public async Task<ActionResult> ChangeUser(string shipid, string olduser, string newuser)
        {
            using (var context = new ApplicationDbContext())
            {
                ...
                var notifyItem = new NotifyItem { RecID = shipid, TypeMessage = NotifyItemMessage.MESSAGE_CHANGEUSER, Body = "הועבר המשלוח " + result.Item1 + " מ" + olduseridLink.FullName + " ל" + newUserLink.FullName, CreatedOn = DateTime.Now, IsRead = false, Title = "העברת משלוח", Url = url };
